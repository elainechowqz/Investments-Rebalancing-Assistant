...
class Investment_category:
    ...
    def __init__(self, name, ratio):
        ...
        self.df = data[data[name] > 0][['ETF/Stock', 'No. of Shares Purchased', 'Full Name', 'MER', 'Summary', 'Volatility', 'Start_date_price', 'End_date_price', 'Percent_change', 'Current Value in CAD']]
    ...
    def MER_plot(self):
        plot_1 = self.df.plot.bar(x='ETF/Stock', y='MER', color='Blue', label='MER_plot')
        
    def Volatility_plot(self):
        plot_2 = self.df.plot.bar(x='ETF/Stock', y='Volatility', color='Green', label='Volatility_plot')
        
    def PercentChange_plot(self):
        plot_3 = self.df.plot.bar(x='ETF/Stock', y='Percent_change', color='Red', label='PercentChange_plot')
        
    def CurrentValue_plot(self):
        plot_4 = self.df['Current Value in CAD'].plot.pie(figsize=(6, 6), label='CurrentValue_pie_plot')
        plot_5 = self.df.plot.bar(x='ETF/Stock', y='Current Value in CAD', color='Orange', label='PercentChange_plot' )
...
rebalance[-1][-1].append(['USA total market Stocks', USA_totalmarket_Stocks.rebalancing()])

USA_bigcap_Stocks = Investment_category('USA Big Cap?', # p)
rebalance[-1][-1].append(['USA big cap Stocks', USA_bigcap_Stocks.rebalancing()])

USA_smallcap_Stocks = Investment_category('USA Small Cap?', # p)
rebalance[-1][-1].append(['USA small Stocks', USA_smallcap_Stocks.rebalancing()])

International_Stocks = Investment_category('International Stocks?', # p)
rebalance[-1].append(['International Stocks', International_Stocks.rebalancing()])

International_allmarkets_Stocks = Investment_category('International All Markets Stocks?', # p)
rebalance[-1][-1].append(['International all markets Stocks', International_allmarkets_Stocks.rebalancing()])

International_developedmarkets_Stocks = Investment_category('International Developed Markets Stocks?', # p)
rebalance[-1][-1].append(['International developed markets Stocks', International_developedmarkets_Stocks.rebalancing()])

International_emergingmarkets_Stocks = Investment_category('International Emerging Markets Stocks?', # p)
rebalance[-1][-1].append(['International emerging markets Stocks', International_emergingmarkets_Stocks.rebalancing()])

Environmental_Stocks = Investment_category('Environmental/ESG Stocks?', # p)
rebalance[-1].append(['Environmental Stocks', Environmental_Stocks.rebalancing()])

Preciousmetal_Stocks = Investment_category('Precious Metal Stocks?', # p)
rebalance[-1].append(['Precious Metal Stocks', Preciousmetal_Stocks.rebalancing()])
